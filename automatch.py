...
parser.add_argument('-i', type=str, help='inputfile', required=True)
parser.add_argument('-r', type=str, help='referencefile', required=True)
parser.add_argument('-o', type=str, help='outputfile', required=True)
...
parser.add_argument('-t', type=str, help='transform type <poly1, poly2, poly3, tps>')


def automatch(inputfile, referencefile, outputfile, feature_name, flann, transform_type,
              gcps, show, tileX=None, tileY=None,
              offsetX=None, offsetY=None, ratio=0.75, binary=False):
    ...
    if not feature_name:
        feature_name = 'brisk'
    img = common.georef(inputfile, referencefile, outputfile, feature_name, flann, transform_type, pointsShp, pointsTxt, tile, offset,
                        ratio, binary)
...
if __name__ == "__main__":
    ...
    img_query, feature_name_query, kp_query, des_query = common.loadOrCompute(args.ki, args.i, args.feature, tile,
                                                                              offset)
    img_train, feature_name_train, kp_train, des_train = common.loadOrCompute(args.kr, args.r, args.feature, tile,
                                                                              offset)
    ...
    projection, gcps, dst_gcps = common.getGCP(args.r, kp_query, kp_train, two_sides_matches, min_matches=3)
    pointsShp = None
    pointsTxt = None
    if gcps:
        pointsShp = args.o + '.shp'
        pointsTxt = args.o + '.points'
    common.saveGeoref(args.i, args.o, projection, args.t, gcps, dst_gcps, pointsShp, pointsTxt)
